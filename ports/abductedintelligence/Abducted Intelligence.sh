...
# variables
GAMEDIR="/$directory/ports/abductedintelligence"
...
if [ "$DISPLAY_WIDTH" -lt 640 ]; then
  ...
  sed -i -E 's/(dpad_mouse_step|deadzone_scale) = [0-9]/\1 = 4/g' abductedintelligence.gptk
elif [ "$DISPLAY_WIDTH" -lt 1280 ]; then
  echo "Setting dpad_mouse_step and deadzone_scale to 5"
  sed -i -E 's/(dpad_mouse_step|deadzone_scale) = [0-9]/\1 = 5/g' abductedintelligence.gptk
elif [ "$DISPLAY_WIDTH" -lt 1920 ]; then
  echo "Setting dpad_mouse_step and deadzone_scale to 6"
  sed -i -E 's/(dpad_mouse_step|deadzone_scale) = [0-9]/\1 = 6/g' abductedintelligence.gptk
else
  echo "Setting dpad_mouse_step and deadzone_scale to 7"
  sed -i -E 's/(dpad_mouse_step|deadzone_scale) = [0-9]/\1 = 7/g' abductedintelligence.gptk
fi

# set up save dir
mkdir -p "$GAMEDIR/savedata"
bind_directories ~/.local/share/ags/Abducted\ Intelligence "$GAMEDIR/savedata"
...
# launch the game
$GPTOKEYB "$GAMEDIR/ags" -c "./abductedintelligence.gptk" &
